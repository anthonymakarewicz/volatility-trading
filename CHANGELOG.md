...
- Runner workflow configs now also support a `named_scenarios` generator with
  reusable `core`, `tail`, and `rr` scenario families, keeping the default
  core set separate from opt-in extreme tail nodes and allowing semantic
  `risk_worst_scenario` labels such as `core.selloff_severe` and
