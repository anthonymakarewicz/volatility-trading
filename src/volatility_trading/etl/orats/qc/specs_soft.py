...
def _get_base_soft_specs() -> list[dict[str, Any]]:
    return [
        # ---- Quote diagnostics ----
        {
...
def _get_exercise_soft_specs(exercise_style: str | None) -> list[dict[str, Any]]:
    if exercise_style == "EU":
        return [
            ...
            },
            {
                ...
                "sample_cols": BASE_KEYS
                + [
                    "call_bid_price","call_mid_price","call_ask_price",
                    "put_bid_price","put_mid_price","put_ask_price",
                ],
            },
        ]
...
def get_soft_specs(
    ...
    config: Any,  # not used here now, but helpful if you want dynamic bins later
) -> list[dict[str, Any]]:
    specs = []
    specs.extend(_get_base_soft_specs())
    specs.extend(
        _get_exercise_soft_specs(
