...
def plot_equity_curve(mtm, sp500):
    ...
    axes[0].plot(
        sp500_rebased.index, sp500_rebased, label="S&P 500 (Rebased)", color="orange"
    )
...
def print_perf_metrics(trades, mtm, risk_free_rate=0.00, alpha=0.01):
    ...
    sharpe_ratio = (
        (daily_returns.mean() - risk_free_rate / 252) / daily_returns.std()
    ) * np.sqrt(252)
...
def plot_full_performance(sp500, mtm_daily):
    ...
    gs = gridspec.GridSpec(4, 2, height_ratios=[2, 1, 1, 1], hspace=0.4, wspace=0.3)

    # ─── row 0: Equity vs SPX ─────────────────────────────
    ax0 = fig.add_subplot(gs[0, :])
    ax0.plot(equity.index, equity, color="tab:blue", label="Equity Curve")
    ax0.plot(
        spx_rebase.index, spx_rebase, color="tab:orange", label="S&P 500 (rebased)"
    )
    ...
    ax1.fill_between(
        spx_dd.index, spx_dd, 0, color="tab:orange", alpha=0.3, label="S&P 500 DD"
    )
...
def print_stressed_risk_metrics(stressed_mtm, daily_mtm, alpha=0.01):
    ...
    total_ret = pd.DataFrame(
        {name: returns + shock_pct[name] for name in shock_pct.columns}
    )
    ...
    stress_es = total_ret.loc[
        total_ret["worst_stressed_ret"] <= stress_var, "worst_stressed_ret"
    ].mean()
