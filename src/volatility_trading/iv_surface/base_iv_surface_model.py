...
class IVSurfaceModel(ABC):
    ...
    def check_arbitrage(
        self, n_k: int = 201, n_T: int = 120, k_pad: float = 0.05, tol: float = 1e-10
    ):
        ...
        T_grid = np.concatenate([T_slice, np.linspace(T_min, T_max, n_T)])
        ...
        report = (
            pd.DataFrame(rows)
            if rows
            else pd.DataFrame(columns=["type", "T", "k", "value"])
        )
    ...
    def plot_smiles(
        ...
        for ax, D in zip(axes, chosen_days):
            ...
            slice_df = slice_df[
                (slice_df["k"] >= k_lo) & (slice_df["k"] <= k_hi)
            ].copy()
            ...
            ax.axvline(0, lw=0.6, c="black", linestyle="--")
